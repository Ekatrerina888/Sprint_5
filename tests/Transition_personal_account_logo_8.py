...
def test_navigation_constructor_and_logo(driver):
    """
    Тест перехода по клику на «Конструктор» и на логотип Stellar Burgers.
    ...
    5. Перейти на страницу конструктора через навигационное меню.
    6. Проверить, что отобразилась страница конструктора.
    7. Вернуться на главную через логотип Stellar Burgers.
    8. Проверить, что отобразилась главная страница.
    """
    ...
    # Шаг 5: Переход на страницу конструктора
    constructor_link = WebDriverWait(driver, 10).until(
        EC.element_to_be_clickable((By.CSS_SELECTOR, CONSTRUCTOR_LINK_BUTTON))
    )
    constructor_link.click()
    logger.info("Выполнен переход по клику на «Конструктор»")

    # Шаг 6: Проверка отображения страницы конструктора
    bun_tab = WebDriverWait(driver, 15).until(
        EC.visibility_of_element_located((By.CSS_SELECTOR, "div.tab_tab__1SPyG:nth-child(1)"))
    )
    assert bun_tab.is_displayed(), "Вкладка 'Булки' не отображается — страница конструктора не загрузилась"
    logger.info("✅ Подтверждено: отобразилась страница конструктора")

    # Шаг 7: Возврат на главную через логотип Stellar Burgers
    ...
    # Шаг 8: Проверка отображения главной страницы
