...
def test_logout_from_personal_cabinet(driver):
    """
    ...
    6. Проверить, что пользователь вернулся на страницу авторизации.
    """
    ...
    # Шаг 5: Выход из аккаунта
    logout_button = WebDriverWait(driver, 10).until(
        EC.element_to_be_clickable((By.CSS_SELECTOR, PERSONAL_CABINET_EXIT_BUTTON))
    )
    logout_button.click()
    logger.info("Выполнен клик на кнопку 'Выйти' в личном кабинете")

    # Шаг 6: Проверка возврата на страницу авторизации
    login_form_title = WebDriverWait(driver, 20).until(
        EC.visibility_of_element_located((By.CSS_SELECTOR, LOGIN_FORM_TITLE_TEXT))
    )
    assert 'Вход' in login_form_title.text, \
        f"Заголовок формы входа не найден. Найденный текст: '{login_form_title.text}'"
    logger.info("✅ Подтверждено: пользователь вышел из аккаунта и вернулся на страницу авторизации")
