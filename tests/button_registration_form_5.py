...
def test_login_via_registration_form(driver):
    ...
    3. Перейти к форме входа через ссылку «Зарегистрироваться» → «Войти».
    4. Заполнить форму входа (email, пароль).
    5. Нажать кнопку «Войти».
    6. Проверить успешность входа (появление кнопки выхода).
    """
    ...
    # Шаг 3: Переход к форме входа через «Зарегистрироваться» и затем «Войти»
    register_link = WebDriverWait(driver, 10).until(
        EC.element_to_be_clickable((By.LINK_TEXT, "Зарегистрироваться"))
    )
    register_link.click()
    logger.info("Перешли на страницу регистрации через кнопку 'Зарегистрироваться'")

    login_link = WebDriverWait(driver, 10).until(
        EC.element_to_be_clickable((By.CSS_SELECTOR, LOGIN_LINK_BUTTON))
    )
    login_link.click()
    logger.info("Нажата кнопка 'Войти' на странице регистрации")
    ...
    # Шаг 6: Проверка успешности входа
