...
# -----------------Этап 3. Проверь переход по клику на логотип Stellar Burgers----------
...
# 6.Найди кнопку "Конструктор" и кликни по ней
constructor_link = WebDriverWait(driver, 25).until(
    EC.element_to_be_clickable((By.XPATH,
        "//*[contains(text(), 'Конструктор') or contains(text(), 'Constructor')]"))
)
...
assert 'Оформить заказ' in order_button.text, \
    f"❌ Ошибка: текст 'Оформить заказ' не найден в кнопке. Текст кнопки: '{order_button.text}'"
print("✅ Нажали на кнопку 'Конструктор'")
